...
ColorsView::MessageReceived(BMessage *message)
{
	switch (message->what) {
		...
		case kColorChanged:
			_ForwardColorChangeToWindow(fColor);
			break;

		default:
			ColorPicker::MessageReceived(message);
	}
}
...
ColorsView::MouseMoved(BPoint where, uint32 code, const BMessage *message)
{
	if (fMouseDown) {
		...
		ColorPicker::MouseMoved(where, code, message);
}
...
void
ColorsView::SetColor(rgb_color base)
{
...
void
ColorsView::_GrabColor()
{
	BScreen screen(Window());
	BWindow* window = new BWindow(screen.Frame(), "", B_NO_BORDER_WINDOW_LOOK,
...
ColorsView::_UpdateColor(float value, float value1, float value2)
{
	...
	_ForwardColorChangeToWindow(base);
}
...
ColorsView::_ForwardColorChangeToWindow(rgb_color color)
{
	...
	BMessage forward = BMessage(kColorChanged);
